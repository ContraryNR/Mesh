...
public slots:
    void onInternalMsg(const QJsonObject& msg)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                //target为1(Coor)时选用"Coor" 而不是使用随'来自Coor.offerER传initialOffer一并传来的localHostName'
                //避免和开启组网还未知Coor的hostName时保存文件(夹)名称不一致
    ...
    void onExternalMsg(const QJsonObject& msg)
    {
        ...
        {
            // Coordinator 通知有 newPeer 加入, 立即创建指向该 newPeer 的 offerER
