    ...
    : QMainWindow(parent), ui(new Ui::MainWindow), mutex(new QMutex)
{
    ...
    connect(ui->btnStart, &QPushButton::clicked, this, [this](){
        ...
        //仅限离线模式使用(虽然在线模式导入也可行,均为共享jsonWorker及下游链路但不建议混用)
        if(dcManager)
...
void MainWindow::initialSignaling()
{
    ...
    if(isCoordinator)
    {
        (serverNetWorker = new servernetworker(localHostName,localHostNum))->moveToThread(trd[NET]=new QThread);
        (coorJsonWorker = new coorjsonworker(localHostName, localHostNum, &serverNetWorker->hostSocketVec, onlineMode))->moveToThread(trd[JS]=new QThread);

        connect(serverNetWorker, &servernetworker::onJsonMsg, coorJsonWorker, &coorjsonworker::onExternalMsg);
        ...
        connect(coorJsonWorker, &coorjsonworker::sendToNetWorker, serverNetWorker, &servernetworker::sendMsg);
    ...
    {
        (clientNetWorker = new clientnetworker)->moveToThread(trd[NET]=new QThread);
        (peerJsonWorker=new peerjsonworker(localHostName, localHostNum, dcManager->nameRoute, onlineMode))->moveToThread(trd[JS]=new QThread);
        ...
        connect(peerJsonWorker, &peerjsonworker::sendToNetWorker, clientNetWorker, &clientnetworker::sendMsg);

        connect(clientNetWorker, &clientnetworker::readySendHostName, peerJsonWorker, &peerjsonworker::onReadySendHostName);
        connect(clientNetWorker, &clientnetworker::onJsonMsg, peerJsonWorker, &peerjsonworker::onExternalMsg);
...
void MainWindow::cleanUp(bool isShutDown)
{
    ...
    if(enCoder)
    {
        QMetaObject::invokeMethod(enCoder, "shutdown", Qt::QueuedConnection);
        trd[EN]->quit();
        trd[EN]->wait();
        delete(trd[EN]);
    ...
    {
        ipRoute=nullptr;
        int forceQuitTicks = 0;
        connect(waitTimer, &QTimer::timeout, this, [this, waitTimer, forceQuitTicks]() mutable {
            forceQuitTicks++;
            if((fileSenderContanier.isEmpty())&&(!dcManager || dcManager->ipRoute.isEmpty()) || forceQuitTicks >= 6)
            {
                waitTimer->stop();
